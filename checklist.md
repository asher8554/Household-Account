...
## 카테고리별 소비 변화 표시 개수 조절

- [x] 연간 소비 추세 화면의 현재 카테고리 표시 제한 구조를 확인한다.
- [x] 카테고리별 소비 변화 패널에 표시 개수 감소/증가 버튼을 추가한다.
- [x] 표시 개수는 최소 1개, 해당 연도 지출 카테고리 수 이하로 제한한다.
...
- [x] `신용`, `본인`이 아닌 신한카드와 현대카드 기준으로 집계하는 실패 테스트를 추가한다.
- [x] 카드별 사용금액 집계를 거래 source의 카드사 라벨 기준으로 수정한다.
- [x] 관련 테스트, 전체 테스트, production build, 브라우저 검증을 통과시킨다.
- [x] 변경사항을 커밋하고 push한다.
...
- [x] 카드별 사용금액 영역과 달력 거래 집계 흐름을 확인한다.
- [x] 선택한 카드사 source만 남기는 실패 테스트를 추가한다.
- [x] 신한카드, 현대카드 토글 버튼 상태를 대시보드에 추가한다.
